...
from crawlee.cacher import (
    html_cacher,
    headless_browser_cacher,
    CachedBeautifulSoupCrawler,
)
...
async def cbsc() -> None:
    # Create an instance of the CachedBeautifulSoupCrawler class, a crawler that automatically
    # gets and parses cached html
    crawler = CachedBeautifulSoupCrawler(
        # On error, retry each page at most once.
        max_request_retries=1,
        # Increase the timeout for processing each page to 30 seconds.
        request_handler_timeout=timedelta(seconds=30),
...
if __name__ == '__main__':
    ...
    #
    # html_urls = ['https://crawlee.dev', 'https://example.com']
    # html_cacher.batch_cache(html_urls)

    # asyncio.run(headless_browser_cacher.browser_cache('https://news.ycombinator.com'))

    # headless_urls = ['https://news.ycombinator.com/ask', 'https://news.ycombinator.com/newest']
    # asyncio.run(headless_browser_cacher.batch_browser_cache(headless_urls))
